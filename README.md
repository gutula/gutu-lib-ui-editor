...
| Primary focus | editor UI, authoring helpers, component composition |
...
| Named Exports | 8 | `packageId`, `packageDisplayName`, `packageDescription`, `editorContentSchema`, `createPlatformEditorExtensions`, `createPlatformEditorConfig`, `usePlatformEditor`, `PlatformEditorContent` |
...
console.log(packageId, packageDisplayName, typeof editorContentSchema);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/ui-editor` if you need lower-level control.
